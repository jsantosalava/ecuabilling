...
function agregarCampania() {
    var tipo = $('#tipo').val();
    var nombreCampania = $('#descripcion').val();
    
    var user = $('#user').val();
    var boton = 'agregarCampania'

    $.ajax({
        url: '../controlador/controladorSpeech.php',
        cache: false,
        type: 'POST',
        data: { nombreCampania, user,tipo, boton },
        ...
        success: function(Resp) {
            ...
            if (Resp === '0') {
                ...
                for (var i = 0; i < gasto.length; i++) {
                    ...
                    html += '<li class="list-group-item d-flex justify-content-between align-items-center" >' + gasto[i][0] + '->'+gasto[i][1]+'<div class="btn-group" role="group" aria-label="Basic example">';
